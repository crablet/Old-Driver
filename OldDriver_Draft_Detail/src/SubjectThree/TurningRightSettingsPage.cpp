...
TurningRightSettingsPage::TurningRightSettingsPage(QWidget *parent) : SettingsPageBase(parent)
{
	InitializeUi();
	InitializeConnect();
}

void TurningRightSettingsPage::mousePressEvent(QMouseEvent *e)
{
	// 点击旁边空白处隐藏设置页面
	if (!this->isHidden())
	{
...
void TurningRightSettingsPage::InitializeConnect()
{
	SettingsPageBase::InitializeConnect();
}

void TurningRightSettingsPage::AddSpecificItemToSettingsTable()
{
	...
	SettingsTable->setCellWidget(2, 0, CreateHeaderLabel("路口右转弯\n评判"));
