...
StraightIntersectionSettingsPage::StraightIntersectionSettingsPage(QWidget *parent) : SettingsPageBase(parent)
{
	InitializeUi();
	InitializeConnect();
}

void StraightIntersectionSettingsPage::mousePressEvent(QMouseEvent * e)
{
	// 点击旁边空白处隐藏设置页面
	if (!this->isHidden())
	{
...
void StraightIntersectionSettingsPage::InitializeConnect()
{
	SettingsPageBase::InitializeConnect();
}

void StraightIntersectionSettingsPage::AddSpecificItemToSettingsTable()
{
	...
	SettingsTable->setCellWidget(SettingsTable->rowCount() - 1, 0, CreateHeaderLabel("直行路口弯\n评判"));
