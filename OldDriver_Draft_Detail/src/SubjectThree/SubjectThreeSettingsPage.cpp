...
SubjectThreeSettingsPage::SubjectThreeSettingsPage(QWidget *parent) : QWidget(parent)
{
	InitializeUi();
	InitializeConnect();
}

bool SubjectThreeSettingsPage::IsAllPagesHidden() const
{
	return PracticeSettingsDetail->isHidden() && DeviceSettingDetail->isHidden();
}

void SubjectThreeSettingsPage::InitializeUi()
{
	...
	VoiceSettingButton->setText("语音/灯光");
	...
	PracticeSettingsDetail = new PracticeSettingsDetailPage(this);
...
void SubjectThreeSettingsPage::InitializeConnect()
{
	// 点击“练习设置”按钮出现设置页
...
void SubjectThreeSettingsPage::SetBackground(const QPixmap &BackgroundPicture)
{
