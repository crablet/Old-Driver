...
PedestrainCrossingSettingsPage::PedestrainCrossingSettingsPage(QWidget *parent) : SettingsPageBase(parent)
{
	InitializeUi();
	InitializeConnect();
}

void PedestrainCrossingSettingsPage::mousePressEvent(QMouseEvent *e)
{
	// 点击旁边空白处隐藏设置页面
	if (!this->isHidden())
	{
...
void PedestrainCrossingSettingsPage::InitializeConnect()
{
	SettingsPageBase::InitializeConnect();
}

void PedestrainCrossingSettingsPage::AddSpecificItemToSettingsTable()
{
	SettingsTable->insertRow(SettingsTable->rowCount());
	SettingsTable->setCellWidget(SettingsTable->rowCount() - 1, 0, CreateHeaderLabel("人行横道\n评判"));
